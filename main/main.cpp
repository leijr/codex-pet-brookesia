...
constexpr const char *CODEX_PET_APP_NAME = "Codex Pet";
...
extern "C" void app_main(void)
{
...

    {
        ...
        for (auto &[name, app] : inited_apps) {
            if ((name == CODEX_PET_APP_NAME) && (app != nullptr) &&
                    (app->getId() >= systems::base::App::APP_ID_MIN)) {
                systems::base::Context::AppEventData event = {
                    .id = app->getId(),
                    .type = systems::base::Context::AppEventType::START,
                    .data = nullptr,
                };
                ESP_UTILS_LOGI("Auto start app: %s (id: %d)", name.c_str(), app->getId());
                ESP_UTILS_CHECK_FALSE_EXIT(phone->sendAppEvent(&event), "Start Codex Pet app failed");
                break;
            }
        }
